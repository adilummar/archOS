...
export default function ClientFilesPage() {
  ...
  if (!project || !firm || !portalSession) return null;

  // Filter files that are shared with client in their latest revision
  const clientFiles = files.filter((f) => {
    if (f.projectId !== projectId) return false;
    const latestRev = f.revisions[f.revisions.length - 1];
    return latestRev?.sharedWithClient;
  });
  ...
  const handleRequestFile = (e: React.FormEvent) => {
    ...
    toast("File request sent to the architect.", "success");
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: "var(--text-base)", color: "var(--color-text-secondary)" }}>
            View and download documents shared by your architect.
          </p>
        </div>
        <button
        ...
        >
          <FileQuestion size={16} /> Request File
        </button>
      </div>
            ...
            <form onSubmit={handleRequestFile} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Latest electrical layout for the kitchen..."
                  required
                  style={{
      ...
      <div style={{
        ...
        {clientFiles.length === 0 ? (
          <div style={{ padding: 48, textAlign: "center", color: "var(--color-text-muted)" }}>
            ...
            <p style={{ margin: 0, fontSize: "var(--text-base)" }}>No files have been shared with you yet.</p>
          </div>
        ) : (
            ...
            <tbody>
              {clientFiles.map(file => (
                <tr key={file.id} style={{ borderBottom: "1px solid var(--color-border)" }}>
                  ...
                  <td style={{ padding: "16px 24px" }}>
                    {file.status === "final" ? (
                      <StatusBadge status="completed" label="Final" />
                    ) : (
